// Adiciona um ou mais registros de PessoaVulnerabilidade
 function createPessoaVulnerabilidade(pessoaVulnerabilidades, database = getDataBase()) {
...
function pessoaVulnerabilidadeAsList(pv) {
    return [
        pv.cpf,
...
// Recupera todos os registros de PessoaVulnerabilidade para um cpf_pessoa
 function recoverPessoaVulnerabilidade(cpf_pessoa, database = getDataBase()) {
    const lista = tableToObject("fPessoaVulnerabilidade", database);
    return lista.filter((pv) => pv.cpf === cpf_pessoa);
}
// Atualiza todos os registros de PessoaVulnerabilidade para um cpf_pessoa
 function updatePessoaVulnerabilidade(cpf_pessoa, novos, database = getDataBase()) {
...
// Remove todos os registros de PessoaVulnerabilidade para um cpf_pessoa
 function deletePessoaVulnerabilidade(cpf_pessoa, database = getDataBase()) {
    const sheet = getSheet("fPessoaVulnerabilidade", database);
    const lista = tableToObject("fPessoaVulnerabilidade", database);
    let linhasRemover = [];
    lista.forEach((pv, idx) => {
