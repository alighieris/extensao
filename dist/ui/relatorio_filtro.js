function getArrayFiltro(cell, sheet) {
    return sheet?.getRange(cell).getValue().split(",").map((s) => s.trim()).filter((i) => i !== undefined && i !== '');
}
function listarPessoasComFiltro() {
    limparListaRelatorio();
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName("Relatório Pessoas");
    let fPessoa = tableToObject("fPessoa").map((p) => {
        return { ...p, idade: utils_calcularIdade(p.data_nascimento) };
    });
    ...
    const dEscolaridade = tableToObject("dEscolaridade");
    ...
    const dVulnerabilidade = tableToObject("dVulnerabilidade");
    ...
    const fPessoaVulnerabilidades = tableToObject("fPessoaVulnerabilidade");
    const dBeneficio = tableToObject("dBeneficio");
    ...
    const fPessoaBeneficio = tableToObject("fPessoaBeneficio");
    const dProjeto = tableToObject("dProjeto");
    const filtro_projeto = getArrayFiltro("C14", sheet).map((f) => { return dProjeto.find((c) => c.nome_projeto == f)?.id; }).filter(i => i !== undefined);
    const fPessoaProjeto = tableToObject("fPessoaProjeto");
    ...
    const resultado = fPessoa.filter(p => {
        ...
        if (filtro_vulnerabilidade.length > 0 && valid) {
            ...
            for (const fv of filtro_vulnerabilidade) {
                if (!vulnerabilidadesPessoa.includes(fv)) {
        ...
        if (filtro_beneficio.length > 0 && valid) {
            ...
            for (const fb of filtro_beneficio) {
                if (!beneficiosPessoa.includes(fb)) {
                    hasBeneficios = false;
                    break;
                }
            }
