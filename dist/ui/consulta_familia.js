function consultarFamiliaPorCPF() {
    ...
    const dParentesco = tableToObject("dParentesco", database);
    if (!pessoa) {
    ...
    if (pessoa.chefe_familia) {
        ...
        chefe = fPessoa.find((p) => p.cpf === pessoa.cpf_chefe_familia);
    }
    const membros = fPessoa.filter((p) => p.cpf_chefe_familia === chefe.cpf);
    const endereco = recoverEndereco(chefe.cpf, database);
    const domicilio = recoverDomicilio(chefe.cpf, database);
...
function limparConsultaFamilia(sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Familia")) {
