 function consultaPessoa() {
    ...
    const dCor = tableToObject("dCor", database);
    const cor = pessoa ? dCor.find((c) => c.id == pessoa.id_cor)?.tipo_cor : "";
    const dEscolaridade = tableToObject("dEscolaridade", database);
    const escolaridade = pessoa ? dEscolaridade.find((e) => e.id == pessoa.id_escolaridade)?.tipo_escolaridade : "";
    const dVulnerabilidade = tableToObject("dVulnerabilidade", database);
    const fPessoaVulnerabilidades = tableToObject("fPessoaVulnerabilidade", database);
...
function limparConsultaPessoa(sheet) {
